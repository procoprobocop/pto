...
3. На компьютере, где планируется запускать скрипты, зайдите: Системные -> Резервное копирование -> Настройки -> Выберите тип снимков (RSYNC) -> Выберите расположение снимков (sdb1) -> ОК
